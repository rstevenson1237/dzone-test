...
func _ready():
    ...
    shape.radius = 2.0  # Smaller than regular projectiles
    ...
    area_shape.radius = 4.0  # Slightly larger detection radius
    ...
    visual.set_script(preload("res://scripts/weapons/LaserVisual.gd"))
    add_child(visual)
...
func _on_detection_area_entered(body: Node2D):
    ...
    print("Laser projectile detected collision with: %s" % body.name)
    
    if body.has_method("take_damage"):
        body.take_damage(damage)
        projectile_hit.emit(body, damage)
        print("Applied %d laser damage to %s" % [damage, body.name])
